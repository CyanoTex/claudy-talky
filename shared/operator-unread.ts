...
export function addPendingUnreadEntries(
  ...
  for (const entry of entries) next.set(entry.messageId, entry);
  return next;
}

export function clearPendingUnreadForAgent(
  current: Map<number, PendingUnreadEntry>,
  agentId: string
): Map<number, PendingUnreadEntry> {
  const next = new Map<number, PendingUnreadEntry>();
  for (const [messageId, entry] of current) {
    if (entry.fromId !== agentId) next.set(messageId, entry);
  }
  return next;
}

export function clearPendingUnreadForConversation(
  current: Map<number, PendingUnreadEntry>,
  conversationId: string
): Map<number, PendingUnreadEntry> {
  const next = new Map<number, PendingUnreadEntry>();
  for (const [messageId, entry] of current) {
    if (entry.conversationId !== conversationId) next.set(messageId, entry);
  }
  return next;
}
